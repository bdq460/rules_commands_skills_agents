...
不需要用户明确声明"我是技术架构师"，只要用户的需求涉及架构设计或技术咨询，就可以调用本技能。
...
在产品开发流程的**阶段6: 架构保障**中被调用，作为技术架构师角色。
...
_`清晰架构(Explicit Architecture)`_：
...
_`COLA架构(Clean Object-Oriented and Layered Architecture)`_：

- 阿里巴巴开源的分层架构框架
- 关注点分离，可扩展性好

_`六边形架构(Hexagonal Architecture)`_：
...
- _`领域驱动设计(DDD)`_：以领域模型为核心，通过限界上下文、聚合根、实体、值对象等概念构建复杂业务系统
- 以领域为中心
- 领域模型驱动设计
...
- _`单一职责原则 (SRP, Single Responsibility Principle)`_：一个类应该只有一个引起它变化的原因，职责分离
- _`开放封闭原则 (OCP, Open-Closed Principle)`_：软件实体对扩展开放，对修改关闭
- _`里氏替换原则 (LSP, Liskov Substitution Principle)`_：子类可以替换父类出现的地方，而不影响程序正确性
- _`接口隔离原则 (ISP, Interface Segregation Principle)`_：客户端不应该依赖它不需要的接口
- _`依赖反转原则 (DIP, Dependency Inversion Principle)`_：高层模块不应依赖低层模块，两者都应依赖抽象

#### 代码质量原则

- _`KISS原则 (Keep It Simple, Stupid)`_：保持简单愚蠢，避免不必要的复杂性
- _`DRY原则 (Don't Repeat Yourself)`_：不要重复自己，避免代码重复
- _`YAGNI原则 (You Aren't Gonna Need It)`_：不要过度设计，只实现当前需要的功能
- _`迪米特法则 (LoD, Law of Demeter)`_：最少知识原则，对象之间应保持最少交互
- _`SoC原则 (Separation of Concerns)`_：关注点分离，将不同关注点分离到不同模块

#### 架构设计原则

- _`高内聚 (Maximize Cohesion)`_：相关的功能应该组织在一起，模块内部元素紧密相关
- _`低耦合 (Minimize Coupling)`_：模块之间依赖关系最小化，降低相互影响
- _`隐藏实现细节 (Hide Implementation Details)`_：通过接口暴露功能，隐藏实现细节
- _`为维护者编码 (Code for the Maintainer)`_：代码应易于理解和维护，不仅是让机器运行

#### 设计决策原则

- _`最小惊讶原则 (POLA, Principle of Least Astonishment)`_：设计应符合用户预期，避免意外行为
- _`最小努力原则 (POLE, Principle of Least Effort)`_：选择最简单有效的解决方案
- _`最后责任时刻 (LRM, The Last Responsible Moment)`_：尽可能推迟不可逆转的决策，在必须做决策时才做
- _`童子军原则 (BSR, Boy-Scout Rule)`_：离开时比来时更干净，留下比发现时更好的代码

#### 优化原则

- _`过早优化是万恶之源 (POITROAE, Premature Optimization Is the Root of All Evil)`_：优先保证正确性和可读性，性能问题在必要时再优化
...
- **主要协作**：所有技术技能（架构约束）、product-coordinator（架构交付）、project-development-flow（架构审查）。
- **RACI（阶段7 架构保障）**：technical-architect 对架构设计和审查负责（R），project-coordinator 对架构质量负责（A），data-engineer、backend-engineer、frontend-engineer、disaster-recovery-planner consulted（C），product-expert 知情并验证（I）。
- **参考**：完整矩阵见 [COLLABORATION_RACI.md](../../COLLABORATION_RACI.md)。

### ⚠️ 冲突升级路径

- **优先自解**：将架构设计分歧同步给 project-coordinator，请求快速复审。
- **二级升级**：若与业务需求或技术策略仍有争议，升级到 project-development-flow 牵头，邀请 product-expert 和相关技术技能共同裁决。
...
- **输入**：业务数据需求、性能要求
- **调用**：data-engineer设计ER图、表结构、索引
- **输出**：完整的数据库Schema、迁移脚本

**场景2**：数据访问层设计

- **输入**：数据模型、查询模式
- **调用**：data-engineer设计Repository模式、数据访问接口
- **输出**：Repository接口定义、数据访问层架构

#### 2. 调用安全工程师（security-engineer）
...
- **输入**：系统架构、安全需求
- **调用**：security-engineer设计认证授权、数据加密、安全通信
- **输出**：安全架构方案、安全策略文档

**场景2**：安全风险评估

- **输入**：系统设计、威胁模型
- **调用**：security-engineer评估安全风险、提供缓解措施
- **输出**：安全风险报告、安全建议清单

#### 3. 调用系统优化师（system-optimizer）
...
const optimizationPlan = await systemOptimizer.optimizePerformance({
...
const disasterPlan = await disasterRecoveryPlanner.designDisasterRecovery({
  rto: recoveryTimeObjective,
  rpo: recoveryPointObjective,
  criticality: businessImpactLevel,
});
...
- **输入**：系统架构、可用性要求
- **调用**：disaster-recovery-planner设计故障切换
- **输出**：故障切换方案、演练计划

### 调用注意事项

1. **调用顺序**：按依赖关系顺序调用，避免循环调用

2. **输入准备**：调用前准备好清晰的输入和上下文信息

3. **输出整合**：将多个技能的输出整合到架构设计文档中

4. **冲突解决**：当不同技能的建议冲突时，以业务目标为优先
...
- **[架构模式](references/architecture-patterns.md)** - 包含架构模式、六边形架构、整洁架构等参考
...
- **Logger工具**：`scripts/utils/logger.ts`
  ...
  const logger = createLogger("Technical Architect");
  logger.info("开始设计系统架构");
  logger.skillComplete("Technical Architect", 5000);
...
- **FileManager工具**：`scripts/utils/file-manager.ts`
  ...
  await fm.createDirectory("./architecture");
  await fm.writeFile("./architecture/design.md", architectureDoc);
...
- **ContextManager工具**：`scripts/utils/context-manager.ts`
  ...
  ctx.set("architecture", "hexagonal");
  ctx.set("techStack", "nodejs+postgres");
...
- **[架构验证器](scripts/architecture-validator.ts)** - 验证代码架构符合性

  ```typescript
  import { ArchitectureValidator } from "./scripts/architecture-validator";
  const validator = new ArchitectureValidator();
  const report = await validator.validate({
    srcDir: "./src",
    rules: ["no-cross-layer", "hexagonal"],
  });
...
本技能负责从技术架构层面确保系统代码符合架构要求,实现健壮性、扩展性、并发支撑性、伸缩性等目标。在产品开发流程中处于阶段6:架构保障,是系统设计的核心。

### 协作的技能类型

本技能主要与以下类型技能协作:

1. **前置技能**: requirements-analyst、product-expert
2. **后置技能**: backend-engineer、frontend-engineer、data-engineer
3. **同级技能**: security-engineer、system-optimizer
4. **依赖技能**: 无
...
| 场景 | 协作技能 | 协作方式 | 协作内容 |
|------|----------|----------|----------|
| 架构实现指导 | backend-engineer | 顺序协作 | 指导后端架构实现 |
| 架构实现指导 | frontend-engineer | 顺序协作 | 指导前端架构实现 |
| 数据架构设计 | data-engineer | 顺序协作 | 协作设计数据架构 |
| 安全架构设计 | security-engineer | 顺序协作 | 协作设计安全架构 |
| 性能架构设计 | system-optimizer | 顺序协作 | 协作设计性能架构 |
| 架构审查 | 所有技能 | 顺序协作 | 审查代码架构合规性 |
...
| 阶段2: 需求分析 | C | 咨询技术可行性 |
| 阶段3: 产品化设计 | C | 咨询技术方案 |
| 阶段6: 架构保障 | R/A | 设计系统架构,指导实现 |
| 阶段7: 测试验证 | I | 了解测试结果,优化架构 |
| 阶段8: 性能优化 | C | 协作优化性能 |
| 阶段10: 安全审查 | C | 协作审查安全 |
| 阶段12: 项目协调与交付 | I | 知晓交付状态 |

### 本技能的核心任务RACI

| 任务 | 本技能 | backend-engineer | data-engineer | security-engineer |
|------|--------|-----------------|---------------|------------------|
| 架构设计 | R/A | C | C | C |
| 架构审查 | R/A | C | C | C |
| 技术选型 | R/A | C | C | C |
...
| 冲突类型 | 推荐协调人 | 原因 |
|----------|-----------|------|
| 技术方案分歧 | project-coordinator | 项目协调权威 |
| 性能要求冲突 | system-optimizer | 性能优化专家 |
| 架构设计冲突 | project-planner | 项目规划专家 |
...
| 选项 | 说明 | 适用场景 |
|------|------|----------|
| 选项A | 继续当前架构 | 架构基本可行,有小的调整 |
| 选项B | 采用备选架构 | 当前架构有明显缺陷 |
| 选项C | 重新设计架构 | 架构存在根本性问题 |
| 选项D | 终止项目 | 项目不可行 |
...
```bash
invoke skill technical-architect
